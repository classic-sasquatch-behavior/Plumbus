...
void absorb_small_blobs_launch(gMat& labels, int threshold) {
	...
	find_sizes_kernel << <num_blocks, threads_per_block >> > (labels, sizes); cusync;
	mark_labels_as_weak_kernel << <num_blocks, threads_per_block >> > (labels, sizes, sizes_mat, weakness, threshold); cusync;
	...
	while (!converged) {
		...
		linear_flow_kernel <<<num_blocks, threads_per_block>>> (labels, temp, sizes_mat, weakness, d_flag); cusync;
